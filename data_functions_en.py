...
def passengers_data(frm, pick=True, height=400, what=['passengers', 'rides', 'fare_hours']):
    ...
    pgs = pgs.with_columns(by_hours.agg(pl.col('fare').sum().alias('fare_hours')))    
    max_jaz, max_pas, max_far = pgs['rides'].max(), pgs['passengers'].max(), pgs['fare_hours'].max()
    coeff = max(max_jaz, max_pas) / max_far
    pgs = pgs.with_columns(pl.col('fare_hours') * coeff)
    trzba = pgs['fare_hours'].sum()
    scalestr = f"Fare multiplied by {coeff:.3f}, total fare {trzba:.2f}" if 'fare_hours' in what else ''
    
    graf = px.bar(pgs, x=column, y=what, barmode='group', height=height, width=750, title=scalestr, 
